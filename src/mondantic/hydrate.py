...
HYDRATE_QUERY = """
query ($board_id: ID!, $cursor: String | null) {
  boards(ids: [$board_id]) {
...
def hydrate(cls: Type[T], api_key: str) -> Iterable[T]:
    """
    ...
    while True:
      res = requests.post(
          ...
          json={
              "query": HYDRATE_QUERY,
              "variables": {"board_id": cls.board_id, "cursor": cursor},
          },
      ...
      parsed = res.json()
      cursor = parsed["data"]["boards"][0]["items_page"]["cursor"]
      ...
      if cursor is None:
        # Keep looping until we've fetched all items
        break
